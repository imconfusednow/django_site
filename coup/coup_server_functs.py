...
def pick_starter(sid):
    ...
    for i in range(4 - len(players)):
        ...
        run_statement("INSERT INTO coup_players (game_id_id, computer, coins, hand, turn, name, player_id,challenged_by, blocked_by, sequence, alive) VALUES(?,?,?,?,?,?,?,?,?,?,?)", [
                      room, 1, 0, '', 0, 'AI ' + random.choice(names), player_id, '', '', 0, 1])
    players = run_query(where, params)
...
def do_action(sid, action_type, blocked, target, cards, room):
    ...
    if not blocked:
        if action_type == "take-1":
            ...
            action_result = coup_action(sid, target, room)
...
def block(sid):
    room = sid_to_room(sid)
    run_statement(
        "UPDATE coup_players SET blocked_by = ? WHERE turn = '1' AND game_id_id = ?", [sid, room])
...
def coup_action(sid, target, room):
    run_statement(
        "UPDATE coup_players SET hand = '' WHERE name = ? AND game_id_id = ?", [target, room])
    run_statement(
        "UPDATE coup_players SET coins = coins - 7 WHERE player_id = ?", [sid])
